...
class dashboard extends Controller
{
    public function home (Request $request)
    {
        ...
        $today=history_daily::whereIn('device_id',$id_device)->whereDay('tanggal_pemakaian',today())->pluck('pemakaian_energi');
        
        $month=history_daily::whereIn('device_id',$id_device)->whereMonth('tanggal_pemakaian',now()->month)->sum('pemakaian_energi');

        $week=history_daily::whereIn('device_id',$id_device)
        ->whereBetween('tanggal_pemakaian',[now()->startOfWeek(), now()->endOfWeek()])
        ->get()->sum('pemakaian_energi');
        return view('dashboard.home',['pemakaian'=> $pemakaian, 'tanggal'=> $tanggal,'today'=>$today[0] ?? 0, 'month'=>$month, 'week'=>$week, 'dropdown_item'=>$dropdown_item]);
    }
